...
configs <- tibble(
  label       = c("Primary Exp(0.1)", "Tighter Gamma(2, 0.1)", "Diffuse Half-Cauchy(25)"),
  prior_type  = c(1, 2, 3),
...
fits <- lapply(seq_len(nrow(configs)), function(i) {
  ...
  mod$sample(
    data = c(dat$stan_data,
             list(prior_type  = configs$prior_type[i],
...
summ <- bind_rows(lapply(seq_along(fits), function(i) {
  fit <- fits[[i]]
  s <- fit$summary(c("mu", "s"), mean, median,
...
# ---- forest plot: posterior mean + 90% CI for mu_g under each config ------
...
p <- ggplot(summ_mu, aes(mean, gene, color = config)) +
  ...
  labs(title = "Prior sensitivity: posterior of gene-level mean rate mu_g",
       subtitle = "points = posterior mean; bars = 90% CI under each s_g prior",
       x = expression(mu[g]), y = NULL, color = NULL) +
  theme_minimal(base_size = 10) +
  theme(legend.position = "bottom")

ggsave("figs/prior_sensitivity.png", p, width = 8, height = 5, dpi = 150)

# print summary table
message("\nposterior means of mu_g under each prior:")
mu_wide <- summ_mu %>%
...
message("\nposterior means of s_g under each prior:")
s_wide <- filter(summ, param == "s") %>%
  select(gene, config, mean) %>%
  pivot_wider(names_from = config, values_from = mean)
print(s_wide)

message("\nsaved figs/prior_sensitivity.png, results/prior_sensitivity.rds")
